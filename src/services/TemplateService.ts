...
export class TemplateService {
    ...
    public async getTemplates(): Promise<CommandTemplate[]> {
        const data = await LocalStorage.getItem<string>(TemplateService.TEMPLATES_KEY);
    ...
    public async saveTemplate(template: CommandTemplate): Promise<void> {
        ...
        await LocalStorage.setItem(TemplateService.TEMPLATES_KEY, JSON.stringify(updated));
    }
        ...
        return [
            {
                id: "transfer-sui",
                name: "Transfer SUI",
                template: "client transfer --to {address} --amount {amount} --gas-budget 10000000",
    ...
    public fillTemplate(template: string, values: Record<string, string>): string {
        let result = template;

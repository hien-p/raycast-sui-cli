...
export class AddressService {
    ...
    public async getAddresses(): Promise<{ address: string; alias: string | null }[]> {
        ...
        try {
            ...
            if (data.addresses && Array.isArray(data.addresses)) {
                return data.addresses.map((addr: any) => {
                    if (Array.isArray(addr)) {
                        ...
                        if (p2 && p2.startsWith("0x")) return { address: p2, alias: p1 || null };
            ...
            return textOutput
                ...
                .filter((item): item is { address: string; alias: string | null } => item !== null);
        }
    }
    ...
    public async createAddress(keyScheme: "ed25519" | "secp256k1" | "secp256r1" = "ed25519", alias?: string): Promise<string> {
    ...
    private async getActiveRpcUrl(): Promise<string | null> {
        try {
            const envsOutput = await this.executor.execute("client envs", { json: true });
            ...
            try {
                ...
                for (const line of lines) {
                    if (line.includes("*")) { // Active env has *
            ...
            try {
                ...
                if (Array.isArray(data)) {
                    ...
                    const total = data.reduce((acc: number, obj: any) => {
                        const bal = obj.mistBalance !== undefined ? obj.mistBalance : (parseInt(obj.balance) || 0);
                        return acc + bal;
                    }, 0);
                ...
                if (gasErrStr.includes("No gas objects") || gasErrStr.includes("No coins") || gasErrStr.includes("not found")) {
                    return "0 SUI";
                }
            }
    ...
    public async getAllBalances(address: string): Promise<{ coinType: string; totalBalance: string; count: number }[]> {
        try {
            const output = await this.executor.execute(`client balance ${address}`, { json: true });
            const data = JSON.parse(output);
            ...
            if (Array.isArray(data) && data.length >= 1 && Array.isArray(data[0])) {
                const groups = data[0];
                const balances: { coinType: string; totalBalance: string; count: number }[] = [];

...
export function AddressList() {
  ...
  return (
    <List
      isLoading={isLoading}
      searchBarPlaceholder="Search addresses..."
      isShowingDetail
    >
      {addresses.map((item) => {
...
${
  item.allBalances.length > 0
    ? item.allBalances
        .map(
          (b) =>
            `| ${b.coinType.split("::").pop()} | ${b.totalBalance} | ${b.count} |`,
        )
        .join("\n")
    : "| No coins found | - | - |"
}
          ...
          <List.Item
              ...
              <List.Item.Detail
                ...
                metadata={
                  <List.Item.Detail.Metadata>
                    ...
                    <List.Item.Detail.Metadata.Label
                      title="Active"
                      icon={isActive ? Icon.Check : Icon.Multiply}
                    />
            ...
            actions={
              <ActionPanel>
                {!isActive && (
                  <Action
                    title="Switch to Address"
                    icon={Icon.Switch}
                    onAction={() => handleSwitch(item.address)}
                  />
                )}
                ...
                <ActionPanel.Section title="Create New Address">
                  <Action
                    title="Create Ed25519"
                    icon={Icon.Plus}
                    onAction={() => handleCreate("ed25519")}
                  />
                  <Action
                    title="Create Secp256k1"
                    icon={Icon.Plus}
                    onAction={() => handleCreate("secp256k1")}
                  />
                  <Action
                    title="Create Secp256r1"
                    icon={Icon.Plus}
                    onAction={() => handleCreate("secp256r1")}
                  />
                </ActionPanel.Section>
              </ActionPanel>
            }

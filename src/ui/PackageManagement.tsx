...
function CreatePackageForm({
  ...
  onPackageCreated: (path: string) => void;
  onCancel: () => void;
}) {
  ...
  const handleSubmit = async () => {
    if (!name || parentPaths.length === 0) {
      showToast({
        ...
        message: "Please provide a name and parent directory",
      });
      return;
    }
...
function PackageActions({ path }: { path: string }) {
  ...
  const runAction = async (
    ...
    try {
      ...
      push(
        <CommandResult result={String(error)} title={`${actionName} Failed`} />,
      );
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <List
      isLoading={isLoading}
      navigationTitle={`Managing: ${path.split("/").pop()}`}
    >
      <List.Section title="Actions">
          ...
          actions={
            <ActionPanel>
              <Action
                title="Run Build"
                onAction={() =>
                  runAction("Building", () => service.build(path))
                }
              />
            </ActionPanel>
          }
        ...
        <List.Item
          ...
          actions={
            <ActionPanel>
              <Action
                title="Run Tests"
                onAction={() => runAction("Testing", () => service.test(path))}
              />
            </ActionPanel>
          }
        />
          ...
          actions={
            <ActionPanel>
              <Action
                title="Publish"
                onAction={() =>
                  push(<PublishForm path={path} service={service} />)
                }
              />
            </ActionPanel>
          }
          ...
          actions={
            <ActionPanel>
              <Action
                title="Upgrade"
                onAction={() =>
                  push(<UpgradeForm path={path} service={service} />)
                }
              />
            </ActionPanel>
          }
          ...
          actions={
            <ActionPanel>
              <Action
                title="Verify"
                onAction={() =>
                  push(<VerifyForm path={path} service={service} />)
                }
              />
            </ActionPanel>
          }
        ...
        <List.Item
          title="Coverage"
          subtitle="Run tests with coverage"
          icon={Icon.BarChart}
          actions={
            <ActionPanel>
              <Action
                title="Run Coverage"
                onAction={() =>
                  runAction("Running Coverage", () =>
                    service.testWithCoverage(path),
                  )
                }
              />
              <Action
                title="Show Summary"
                onAction={() =>
                  runAction("Coverage Summary", () =>
                    service.coverageSummary(path),
                  )
                }
              />
            </ActionPanel>
          ...
          actions={
            <ActionPanel>
              <Action
                title="Migrate"
                onAction={() =>
                  runAction("Migrating", () => service.migrate(path))
                }
              />
            </ActionPanel>
          }
        ...
        <List.Item
          title="Disassemble"
          subtitle="Disassemble module bytecode"
          icon={Icon.Code}
          actions={
            <ActionPanel>
              <Action
                title="Disassemble"
                onAction={() =>
                  push(<DisassembleForm path={path} service={service} />)
                }
              />
            </ActionPanel>
          }
...
function UpgradeForm({
  ...
  return (
    <Form
      ...
      actions={
        <ActionPanel>
          <Action.SubmitForm title="Upgrade Package" onSubmit={handleSubmit} />
        </ActionPanel>
      }
    >
      <Form.TextField
        id="capId"
        title="Upgrade Capability ID"
        value={capId}
        onChange={setCapId}
        placeholder="0x..."
      />
...
function VerifyForm({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      push(
        <CommandResult result={String(error)} title="Verification Failed" />,
      );
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <Form
      ...
      actions={
        <ActionPanel>
          <Action.SubmitForm title="Verify Source" onSubmit={handleSubmit} />
        </ActionPanel>
      }
    >
      <Form.TextField
        id="pkgAddr"
        title="Package Address"
        value={pkgAddr}
        onChange={setPkgAddr}
        placeholder="0x..."
      />
...
function DisassembleForm({
  ...
  const handleSubmit = async () => {
    setIsLoading(true);
    try {
      showToast({ style: Toast.Style.Animated, title: "Disassembling..." });
      const result = await service.disassemble(path, moduleName);
      showToast({ style: Toast.Style.Success, title: "Disassembled!" });
      push(<CommandResult result={result} title="Disassemble Output" />);
    } catch (error) {
      showToast({
        style: Toast.Style.Failure,
        title: "Disassembly Failed",
        message: String(error),
      });
      push(<CommandResult result={String(error)} title="Disassembly Failed" />);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <Form
      ...
      actions={
        <ActionPanel>
          <Action.SubmitForm title="Disassemble" onSubmit={handleSubmit} />
        </ActionPanel>
      }
    >
      <Form.TextField

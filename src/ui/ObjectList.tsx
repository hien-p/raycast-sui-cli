...
export function ObjectList() {
  ...
  useEffect(() => {
    async function fetchObjects() {
      try {
        const data =
          await commandService.executeCommandJson<SuiObject[]>(
            "client objects",
          );
        setObjects(data);
      } catch (error) {
  ...
  const filteredObjects = objects.filter((obj) => {
    ...
    if (filterType === "coins")
      return obj.data.type.startsWith("0x2::coin::Coin");
  ...
  return (
    ...
    >
      {filteredObjects.map((obj) => {
        ...
        const balance =
          isCoin && obj.data.content?.fields?.balance
            ? `${obj.data.content.fields.balance} MIST`
            : null;
              ...
              <List.Item.Detail
                metadata={
                  <List.Item.Detail.Metadata>
                    ...
                    <List.Item.Detail.Metadata.Label
                      title="Object ID"
                      text={obj.data.objectId}
                    />
                    <List.Item.Detail.Metadata.Label
                      title="Version"
                      text={obj.data.version}
                    />
                    <List.Item.Detail.Metadata.Label
                      title="Digest"
                      text={obj.data.digest}
                    />
                    {balance && (
                      <List.Item.Detail.Metadata.Label
                        title="Balance"
                    ...
                    {obj.data.content?.fields &&
                      Object.entries(obj.data.content.fields).map(
                        ([key, value]) => (
                          <List.Item.Detail.Metadata.Label
                            key={key}
                            title={key}
            ...
            actions={
              <ActionPanel>
                <Action.Push
                  title="View Details"
                  icon={Icon.Eye}
                  target={<ObjectDetail object={obj.data} />}
                />
                <Action.CopyToClipboard
                  content={obj.data.objectId}
                  title="Copy Object Id"
                />
                <Action.CopyToClipboard
                  content={obj.data.type}
                  title="Copy Type"
                />
                {balance && (
                  <Action.CopyToClipboard
                    content={obj.data.content.fields.balance}
                    title="Copy Balance"
                  />
                )}
                <Action.CopyToClipboard

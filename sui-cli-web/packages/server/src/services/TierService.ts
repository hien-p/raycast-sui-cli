...
export class TierService {
  ...
  async getTransactionCount(address: string): Promise<number> {
    ...
    try {
      // Query transactions from address (as sender)
      const response = await fetch(rpcUrl, {
        ...
        body: JSON.stringify({
          ...
          params: [
            ...
            null, // cursor
            1, // limit (we just need count, but API returns total)
            true, // descending
          ],
        }),
      });
      ...
      // Note: Sui RPC doesn't directly return total count, so we estimate
      // For accurate count, we'd need to paginate through all results
      // For MVP, we'll use a heuristic based on hasNextPage
      const data = result.result?.data || [];
      const hasMore = result.result?.hasNextPage || false;

      // If we got data and there's more, make another call to estimate
      if (hasMore) {
        // Query with higher limit to get better estimate
        ...
        const countResult = (await countResponse.json()) as {
          result?: { data?: unknown[]; hasNextPage?: boolean };
        };

        const countData = countResult.result?.data || [];
        const stillHasMore = countResult.result?.hasNextPage || false;

        // If still has more after 100, estimate based on pattern
        // This is an approximation - for production, consider using indexer
        if (stillHasMore) {
          return countData.length + 50; // Conservative estimate
        }
